...
class OrderScooterPage(BasePage):
    ...
    def enter_name(self, name):
        self._find_element(locators.NAME_LOCATOR).send_keys(name)

    @allure.step("Ввести фамилию: {surname}")
    def enter_surname(self, surname):
        self._find_element(locators.SURNAME_LOCATOR).send_keys(surname)

    @allure.step("Ввести адрес: {address}")
    def enter_address(self, address):
        self._find_element(locators.ADDRESS_LOCATOR).send_keys(address)

    @allure.step("Выбрать станцию метро: {station_name}")
    def choose_subway_station(self, station_name):
        element = self._find_element(locators.SUBWAY_STATION_LOCATOR)
        element.click()
        element.send_keys(station_name)
        station_option_locator = self._subway_station_option_locator(station_name)
        self._wait_until_visible(station_option_locator).click()

    @allure.step("Ввести телефон: {phone}")
    def enter_phone(self, phone):
        self._find_element(locators.PHONE_LOCATOR).send_keys(phone)

    @allure.step("Нажать кнопку 'Далее'")
    def push_next_button(self):
        self._find_element(locators.NEXT_BUTTON_LOCATOR).click()
        self._wait_until_visible(locators.ABOUT_RENT_HEADER_LOCATOR)

    @allure.step("Ввести дату доставки: {date}")
    def enter_delivery_date(self, date):
        element = self._find_element(locators.DELIVERY_DATE_LOCATOR)
        element.send_keys(date)
        element.send_keys(Keys.ESCAPE)

    @allure.step("Выбрать срок аренды: {rent_period_name}")
    def choose_rent_period(self, rent_period_name):
        self._find_element(locators.RENT_PERIOD_LOCATOR).click()
        option_locator = self._rent_period_option_locator(rent_period_name)
        self._wait_until_visible(option_locator).click()

    @allure.step("Выбрать цвета: {colors_ids}")
    def choose_colors(self, colors_ids):
        for id in colors_ids:
            self._find_element(self._color_option_locator(id)).click()

    @allure.step("Ввести комментарий: {comment}")
    def enter_comment(self, comment):
        self._find_element(locators.COMMENT_LOCATOR).send_keys(comment)

    @allure.step("Нажать кнопку 'Заказать'")
    def push_order_button(self):
        self._find_element(locators.FINISH_ORDER_BUTTON).click()
        self._wait_until_visible(locators.ORDER_CONFIRMATION_HEADER_LOCATOR)

    @allure.step("Подтвердить заказ")
    def complete_order(self):
        self._find_element(locators.ORDER_CONFIRMATION_BUTTON_LOCATOR).click()
        element = self._wait_until_visible(locators.ORDER_DONE_HEADER_LOCATOR)
